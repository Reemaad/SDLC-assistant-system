...
const GROUP = "sprint-ifo";

// Create the client and turn down the library's own chatter.
const kafka = new Kafka({ clientId: "jira-integration", brokers: [BROKER], logLevel: logLevel.ERROR });
...
function log(fields) {
  const line = { ts: new Date().toISOString(), service: "jira-integration" };
...
async function main() {
  ...
  await consumer.run({
    eachMessage: async function (payload) {
      ...
      if (evt.type !== "CallCaptured") {
        log({ level: "info", event: "type_ignored", type: evt.type });
        return;
      }

      // Find the current status for this sprint, or zero if we do not know it.
    //   const before = sprintInfo[evt.status] || 0;
    ...
    //   stock[evt.sku] = after;
      ...
      log({
        ...
        sku: evt.sku,
        status: evt.status,
        sprint_info: sprintInfo[evt.sprint_id]
    });
  }
});
}
